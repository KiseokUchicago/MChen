...
plot(assay(airway)[,2:3], cex=.1)
plot(assay(airway)[,c(1,8)],cex=.1)
assay(airway)
...
msd$gg + ggtitle("") + scale_y_continuous(limits = c(0, 3))

msd <- meanSdPlot(assay(rld), ranks = FALSE)
msd$gg + ggtitle("") + scale_y_continuous(limits = c(0, 3))
...
plot(hclust(dist(t(assay(vsd)))), labels=colData(dds)$dex)
plot(hclust(dist(t(assay(rld)))), labels=colData(dds)$dex)
